...
include("mrt_solution_algorithms.jl") 
include("network_augmentation.jl")
include("network_statistics_and_modification.jl")

rng = Random.MersenneTwister(182) #reset random seed

Bus_location_list = CSV.read("Greensboro_US_buses.csv",DataFrame)
Line_list = CSV.read("Greensboro_US_lines.csv",DataFrame)
...
max_switches = Dict{Tuple{Int, Int, Int}, Vector}() ##SWATI added
red_greedy_switches = Dict{Tuple{Int,Int,Int}, Tuple{Vector{Any}, Vector{Float64}, Vector{Float64}, Float64}}() ##SWATI added
